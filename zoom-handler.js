...
class ZoomHandler {
  constructor(element) {
    ...
    // Try to find the scroller, but it's okay if it's null initially
  ...
  /**
   * Made safe for finding the scroller
   */
  ...
  handleTouchStart(e) {
    if (e.touches.length === 2) {
      ...
      // If scroller not found, don't start zoom
      if (!this.pageOrigin) return; 

      // Only start zoom if coordinates are valid
      this.panning = true; 
      ...
      // If scroller not found, don't start pan
      if (!trueHit) return; 

      // Only start pan if coordinates are valid
  ...
  handleTouchMove(e) {
    ...
    // Panning (1 finger drag)
      ...
      const trueHit = this.getPageCoordinates(e.touches);
      if (!trueHit) return; // Extra safety
